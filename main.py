...
HIGH_PRIORITY = add_png_suffix(
    config.get("high_priority", DEFAULT_CONFIG["high_priority"])
)
LOW_PRIORITY = add_png_suffix(
    config.get("low_priority", DEFAULT_CONFIG["low_priority"])
)
...
def setup_logging():
    ...
    file_handler = RotatingFileHandler(
        "afm_macro.log", maxBytes=1024 * 1024, backupCount=3, encoding="utf-8"
    )
...
def get_wave_region(window):
    return {
        ...
        "left": window.left
        + int(window.width * 0.32),  # Not ideal since only works in 1080p
...
def preprocess_wave_image(image):
    ...
    scale_factor = 5
    gray = cv2.resize(
        gray, (image.shape[1] * scale_factor, image.shape[0] * scale_factor)
    )
    ...
    thresh = cv2.adaptiveThreshold(
        blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 7
    )
    ...
    while attempts < max_attempts:
        try:
            ...
            regions = [
                ...
                (
                    first_card_left + 2 * (card_width + gap),
                    0,
                    card_width,
                    window_height,
                ),
                ...
                try:
                    ...
                    for upgrade in UPGRADE_PRIORITY:
                        ...
                        res = cv2.matchTemplate(
                            card_img, scaled_template, cv2.TM_CCOEFF_NORMED
                        )
...
def get_rarity_from_color(card_img, position):
    try:
        ...
        sample_region = card_img[
            y_base - 2 : y_base + 3, scan_x : scan_x + sample_width
        ]
        ...
        if DEBUG:
            log.debug(
                f"Position {position} | Scanned at X={scan_x} | Color: {rgb_color}"
            )
        ...
        for ref_color, rarity in RARITY_COLORS.items():
            distance = (
                sum((c1 - c2) ** 2 for c1, c2 in zip(ref_color, rgb_color)) ** 0.5
            )
...
def select_best_upgrade(upgrades):
    ...
    for upgrade in upgrades:
        ...
        if "unit" in upgrade["upgrade"].lower() and upgrade["rarity"] == "Common":
            if DEBUG:
                log.debug(
                    f"Skipping Common unit upgrade: {upgrade['upgrade']} at pos {upgrade['position']}"
                )
            continue

        # Skip flat ATK/HP upgrades (non-percent)
        if upgrade["original_upgrade"] in ["atk.png", "health.png"] and not upgrade.get(
            "is_percent", False
        ):
            if DEBUG:
                log.debug(
                    f"Skipping flat {upgrade['original_upgrade']} upgrade at pos {upgrade['position']}"
                )
            continue
    ...
    if DEBUG:
        ...
        for idx, upgrade in enumerate(valid_upgrades, 1):
            ...
            log.debug(
                f"{idx}. [{group}] {upgrade['upgrade']}{perc} ({upgrade['rarity']}) at pos {upgrade['position']}"
            )
    ...
    for upgrade in valid_upgrades:
        ...
        if DEBUG:
            log.debug(
                f"Attempting: {upgrade['upgrade']}{perc} at position {upgrade['position']}"
            )
        if navigate_to(upgrade["position"]):
            record_upgrade_purchase(upgrade["upgrade"], upgrade["rarity"])
            clean_name = (
                upgrade["upgrade"].replace(".png", "").replace("IMAGES\\", "").upper()
            )
...
def generate_upgrade_summary():
    ...
    RARITY_EMOJIS = {"Mythic": "🔴", "Legendary": "🟡", "Epic": "🟣", "Common": "🔵"}
...
def main_loop():
    ...
    log.info(
        f"Mode: {MODE.capitalize()} Scan | Press {START_KEY} to begin."
        if not AUTO_START
        else "Auto-start: Enabled."
    )
    log.info(f"Press {PAUSE_KEY} to pause/resume | {STOP_KEY} to stop")
    log.info(f"Money Mode: {'ENABLED' if MONEY_MODE else 'disabled'}")
    log.info(
        f"High priority upgrades: {', '.join([upgrade.replace('.png', '') for upgrade in HIGH_PRIORITY])}"
    )
    log.info(
        f"Low priority upgrades: {', '.join([upgrade.replace('.png', '') for upgrade in LOW_PRIORITY])}"
    )
                ...
                if window:
                    ...
                    if upgrades:
                        ...
                        if current_wave:
                            ...
                            if (
                                current_wave < DELAY_START
                                or (current_wave % 10) in ALLOWED_ROUNDS
                            ):
                                if last_upgraded_wave != current_wave:
                                    if focus_roblox_window():
                                        if select_best_upgrade(upgrades):
                                            last_upgraded_wave = (
                                                current_wave  # Record this wave
                                            )
                                            log.info(
                                                f"Upgrade executed on wave {current_wave}"
                                            )
                                        else:
                                    ...
                                    if DEBUG:
                                        log.info(
                                            f"Already upgraded on wave {current_wave}. Skipping."
                                        )
                                    last_scan = time.time()
                            else:
                                if DEBUG:
                                    log.info(
                                        f"Upgrade delay active for wave {current_wave}"
                                    )
                                last_scan = time.time()
                        else:
